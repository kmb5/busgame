...
export const Card = ({ card, faceDown = false, isActive = false, flipped = true, animateMove = false }: CardProps) => {
  const suitColor = getSuitColor(card.suit);
  const suitSymbol = getSuitSymbol(card.suit);
  const ringClass = isActive ? 'ring-4 ring-yellow-400' : '';

  // Flip animation classes
  // flipped=false: show back; flipped=true: show front
  const flipClass = `transition-transform duration-500 transform-style-preserve-3d ${flipped ? '' : 'rotate-y-180'}`;

  return (
    <div className={`w-[min(20vw,120px)] h-[min(28vw,168px)] ${ringClass} ${animateMove ? 'transition-transform duration-500' : ''}`}
      style={{ perspective: '800px' }}>
      <div className={`relative w-full h-full ${flipClass}`}
        style={{ position: 'relative' }}>
        {/* Card Back (face down) */}
        <div className={`absolute w-full h-full backface-hidden rotate-y-180 z-10`}
